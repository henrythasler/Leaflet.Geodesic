...
describe("Helper functions", function () {
    it("toDegrees() - convert radians to degrees", function () {
        expect(geodesic.toDegrees(0)).toBeCloseTo(0, closeToDigits_5);
        expect(geodesic.toDegrees(Math.PI)).toBeCloseTo(180, closeToDigits_5);
        expect(geodesic.toDegrees(-2 * Math.PI)).toBeCloseTo(-360, closeToDigits_5);
    });

    it("toRadians() - convert degrees to radians", function () {
        expect(geodesic.toRadians(0)).toBeCloseTo(0, closeToDigits_5);
        expect(geodesic.toRadians(180)).toBeCloseTo(Math.PI, closeToDigits_5);
        expect(geodesic.toRadians(-360)).toBeCloseTo(-2 * Math.PI, closeToDigits_5);
    });

    it("wrap360() - limit value to 0..360", function () {
        expect(geodesic.wrap360(90)).toBeCloseTo(90, closeToDigits_5);
        expect(geodesic.wrap360(360)).toBeCloseTo(0, closeToDigits_5);
        expect(geodesic.wrap360(-90)).toBeCloseTo(270, closeToDigits_5);
    });

    it("wrap() - limit value to -360...+360", function () {
        expect(geodesic.wrap(90)).toBeCloseTo(90, closeToDigits_5);
        expect(geodesic.wrap(360)).toBeCloseTo(360, closeToDigits_5);
        expect(geodesic.wrap(-360)).toBeCloseTo(-360, closeToDigits_5);
        expect(geodesic.wrap(-90)).toBeCloseTo(-90, closeToDigits_5);
        expect(geodesic.wrap(1000)).toBeCloseTo(280, closeToDigits_5);
        expect(geodesic.wrap(-1000)).toBeCloseTo(-280, closeToDigits_5);
    });

    it("wrap(?, 180) - limit value to -180...+180", function () {
        expect(geodesic.wrap(90, 180)).toBeCloseTo(90, closeToDigits_5);
        expect(geodesic.wrap(360, 180)).toBeCloseTo(0, closeToDigits_5);
        expect(geodesic.wrap(-360, 180)).toBeCloseTo(0, closeToDigits_5);
        expect(geodesic.wrap(180, 180)).toBeCloseTo(180, closeToDigits_5);
        expect(geodesic.wrap(-180, 180)).toBeCloseTo(-180, closeToDigits_5);
        expect(geodesic.wrap(-90, 180)).toBeCloseTo(-90, closeToDigits_5);
        expect(geodesic.wrap(1000, 180)).toBeCloseTo(-80, closeToDigits_5);
        expect(geodesic.wrap(-1000, 180)).toBeCloseTo(80, closeToDigits_5);
    });
...
describe("Vincenty direct - regular test cases", function () {
    it("Along the equator", function () {
        ...
        expect(dest.lat).toBeCloseTo(0, closeToDigits_5);
        expect(dest.lng).toBeCloseTo(0.898315, closeToDigits_5);  // position was validated with QGIS-Plugin ("Create line of bearing")
        expect(dest.bearing).toBeCloseTo(90, closeToDigits_5);
    });
    ...
    it("Equator circumference", function () {
        ...
        expect(dest).toContainAllKeys(["lat", "lng", "bearing"]);
        expect(dest.lat).toBeCloseTo(0, closeToDigits_5);
        expect(dest.lng).toBeCloseTo(0, closeToDigits_5);
        expect(dest.bearing).toBeCloseTo(90, closeToDigits_5);
    });
    ...
    it("FlindersPeak to Buninyong", function () {
        ...
        expect(dest).toBeObject();
        expect(dest).toContainAllKeys(["lat", "lng", "bearing"]);
        expect(dest.lat).toBeCloseTo(Buninyong.lat, closeToDigits_5);
        expect(dest.lng).toBeCloseTo(Buninyong.lng, closeToDigits_5);
        expect(dest.bearing).toBeCloseTo(307. + 10. / 60. + 25.07 / 3600., closeToDigits_5);
    });
    ...
    it("Let's go somewhere from Berlin", function () {
        ...
        expect(dest).toBeObject();
        expect(dest).toContainAllKeys(["lat", "lng", "bearing"]);
        expect(dest.lat).toBeCloseTo(-40.018704, closeToDigits_5);
        expect(dest.lng).toBeCloseTo(143.167929, closeToDigits_5);  // position was validated with QGIS-Plugin ("Create line of bearing")
        expect(dest.bearing).toBeCloseTo(127.299753, closeToDigits_5);
    });
});
...
describe("Vincenty direct - Corner-cases and error handling", function () {
    it("antipodal position along equator", function () {
        ...
        expect(dest).toContainAllKeys(["lat", "lng", "bearing"]);
        expect(dest.lat).toBeCloseTo(0, closeToDigits_5);
        expect(dest.lng).toBeCloseTo(-180, closeToDigits_5);  // FIXME: why is this not +180?
        expect(dest.bearing).toBeCloseTo(90, closeToDigits_5);
    });
...
describe("Vincenty inverse - regular test cases", function () {
    it("FlindersPeak to Buninyong", function () {
        ...
        expect(res.distance).toBeCloseTo(54972.271, closeToDigits_3);   // epsilon is larger, because precision of reference value is  only 3 digits
        expect(res.initialBearing).toBeCloseTo(306 + 52 / 60 + 5.37 / 3600, closeToDigits_5);
        expect(res.finalBearing).toBeCloseTo(307 + 10. / 60. + 25.07 / 3600., closeToDigits_5);
    });
    ...
    it("Berlin to LosAngeles", function () {
        ...
        expect(res.finalBearing).toBeCloseTo((180 + 27) % 360 + (11 + 60) % 60 / 60. + (8.62 + 60) % 60 / 3600., closeToDigits_5);
    });
...
describe("Vincenty inverse - Corner-cases and error handling", function () {
    ...
    it("Coincident (start=destination)", function () {
        ...
        expect(res).toBeObject();
        expect(res).toContainAllKeys(["distance", "initialBearing", "finalBearing"]);
        expect(res.distance).toBeCloseTo(0, closeToDigits_3); // verified via http://www.ga.gov.au/geodesy/datums/vincenty_inverse.jsp
    ...
    it("mitigate convergence Error", function () {
        ...
        expect(res).toBeObject();
        expect(res).toContainAllKeys(["distance", "initialBearing", "finalBearing"]);
        expect(res.distance).toBeCloseTo(19969603.453263, closeToDigits_5);
        expect(res.initialBearing).toBeCloseTo(90, closeToDigits_5);   // the formula has special handling for antipodals (going only north/south)
        expect(res.finalBearing).toBeCloseTo(90, closeToDigits_5);
    });
...
describe("Intersection - regular test cases", function () {
    it("Intersect at zero island", function () {
        ...
        if (res) {
            ...
            expect(res.lat).toBeCloseTo(0, closeToDigits_5);
            expect(res.lng).toBeCloseTo(0, closeToDigits_5);
        }
        else {
    ...
    it("Intersection Berlin-Santiago and Seattle-Capetown", function () {
        ...
        if (res) {
            expect(res).toBeInstanceOf(L.LatLng);
            expect(res.lat).toBeCloseTo(17.099091, closeToDigits_5);  // checked with QGIS
            expect(res.lng).toBeCloseTo(-33.681335, closeToDigits_5);
        }
        else {
            expect.fail("Expected intersection to be found, but got null");
        }
    ...
    it("Intersection from Chris Veness (stn-cdg-bxl)", function () {
        ...
        if (res) {
            expect(res).toBeInstanceOf(L.LatLng);
            expect(res.lat).toBeCloseTo(50.9078, closeToDigits_4);
            expect(res.lng).toBeCloseTo(4.5084, closeToDigits_4);
        }
        else {
            expect.fail("Expected intersection to be found, but got null");
        }
...
describe("Intersection - Corner-cases and error handling", function () {
    it("Coincident", function () {
        ...
        if (res) {
            expect(res).toBeInstanceOf(L.LatLng);
            expect(res.lat).toBeCloseTo(Berlin.lat, closeToDigits_5);
            expect(res.lng).toBeCloseTo(Berlin.lng, closeToDigits_5);
        }
        else {
            expect.fail("Expected intersection to be found, but got null");
        }
    ...
    it("Over southpole with φ3=NaN", function () {
        ...
        if (res) {
            expect(res).toBeInstanceOf(L.LatLng);
            expect(res.lat).toBeCloseTo(-90, closeToDigits_5);
            expect(res.lng).toBeCloseTo(196.00983852008366, closeToDigits_5);
        }
        else {
            expect.fail("Expected intersection to be found, but got null");
        }
...
describe("midpoint - regular test cases", function () {
    it("Seattle and Capetown", function () {
        ...
        expect(res).toBeInstanceOf(L.LatLng);
        expect(res.lat).toBeCloseTo(18.849527, closeToDigits_5);
        expect(res.lng).toBeCloseTo(-35.885828, closeToDigits_5);
    });
});
